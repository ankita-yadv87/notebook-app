...
const Notes = (props) => {
    ...
    return (
        <>
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-footer">
                            ...
                            <button disabled={note.etitle.length < 4 || note.edescription.length < 4} type="button" className="btn btn-primary" onClick={handleClick} >Update Note</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="row my-3" style={{backgroundColor: props.mode === 'dark'? "#05152c" : "white" , color: props.mode === 'dark'? 'white': 'black'} }>
                <h2> Your Notes </h2>
                <div className="container mx-2" >
                    {notes.length === 0 && 'No Note to display'}
                </div>
                {notes.map((note) => {
                    return <Noteitem key={note._id} mode={props.mode} updateNote={updateNote} note={note} showAlert={props.showAlert} />;
                })}
            </div>
        </>
    )
